...
from fairseq.data.data_utils import MultiClassSampler
...
LANGUAGE_SAMPLER_CHOICES = ChoiceEnum(["uniform_sampler", "alpha_sampler"])
...
@dataclass
class MultilingualLanguageModelingConfig(CheckpointConfig):
    ...
    one_language_per_batching: Optional[bool] = field(
        default=False,
        metadata={
            "help": "If True, during training, each batch only has sequences from a single language (default: False)"
        },
    )

    sampling_func: LANGUAGE_SAMPLER_CHOICES = field(
        default="uniform_sampler",
        metadata={
            "help": "A function for sampling over the multilingual datasets. "
            "Only used when one_language_per_batching is True"
            "The default strategy is to sample uniformly. (default: uniform_sampler)"
        },
    )

    local_sampler_seed: Optional[int] = field(
        default=1,
        metadata={
            "help": "The sampling funcion is somehow not seeded by the default fairseq seed. This is a bit hacky fix to seed randomness (default: 1)"
        },
    )
...
class MultilingualLanguageModelingTask(LegacyFairseqTask):
    ...
    def load_dataset(self, split: str, epoch=1, combine=False, **kwargs):
        ...
        for lang_id, language in enumerate(languages):
            if (
                self.args.one_language_per_batching
                and split == self.args.train_subset
                and language not in self.lang2id
            ):
                self.lang2id[language] = lang_id
                self.id2lang[lang_id] = language
        ...
        if split == self.args.train_subset:
            logger.info(f"Saving meta-data: Lang2ID, ID2Lang")
            json.dump(self.lang2id, open(f"{self.args.save_dir}/lang2id.json", "w"))
            json.dump(self.id2lang, open(f"{self.args.save_dir}/id2lang.json", "w"))
        logger.info(f"Lang2ID: {self.lang2id}")
        logger.info(f"ID2Lang: {self.id2lang}")
        ...
        if split == self.args.train_subset:
            ...
            if self.args.one_language_per_batching:
                logger.info(
                    "Enforce each GPU to receive sequences from only one language"
                )
                dic = OrderedDict()
                for i, lang in enumerate(languages):
                    dic[lang] = lang_datasets[i]
                if self.args.sampling_func == "uniform_sampler":
                    logger.info("Sample each languages uniformly")
                    sampling_func = MultiClassSampler()
                elif self.args.sampling_func == "alpha_sampler":
                    ...
                    sampling_func = MultiClassSampler(
                        self.args.local_sampler_seed, sample_probs
                    )
                else:
                    raise NotImplementedError(
                        f"Sampling function: {self.args.sampling_func} is not supported"
                    )

                dataset = MultiCorpusSampledDataset(dic, sampling_func=sampling_func)
            else:
                # For train subset, additionally up or down sample languages.
                sample_probs = self._get_sample_prob(true_dataset_lengths)
                ...
                size_ratio = (
                    sample_probs * true_dataset_lengths.sum()
                ) / dataset_lengths
        ...
        if split != self.args.train_subset or not self.args.one_language_per_batching:
