...
class TestCheckpointUtils(unittest.TestCase):
    ...
    @unittest.skip("Disabled on gshard temporarily")
    def test_s3_save_load(self):
        state_dict = {"hello": "sam"}
        filename = "async_checkpoint.pt"
        s3_dir = "s3://fairusersglobal/users/sshleifer/test/"
        expected_s3_path = os.path.join(s3_dir, filename)
        assert not PathManager.exists(expected_s3_path)

        def upload_and_remove_local_copy(local_path, s3_upload_path):
            PathManager.copy_from_local(
                local_path, os.path.join(s3_upload_path, local_path), overwrite=True
            )
            PathManager.rm(local_path)
            assert not PathManager.exists(local_path)

        def async_callback_fn(filename):
            upload_and_remove_local_copy(filename, s3_dir)
            assert PathManager.exists(expected_s3_path)

        checkpoint_utils.torch_persistent_save(
            state_dict, filename, async_write=True, async_callback_fn=async_callback_fn
        )

        PathManager.async_close()
        assert not PathManager.exists(filename)
        PathManager.rm(expected_s3_path)
        assert not PathManager.exists(expected_s3_path)
