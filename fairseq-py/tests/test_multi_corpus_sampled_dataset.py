...
class TestMultiCorpusSampledDataset(unittest.TestCase):
    def setUp(self):
        ...
        self.indexed_monolingual_dataset_1 = MonolingualDataset(
            tokens_1,
            sizes=[tokens_1.size(-1)],
            src_vocab=d,
            add_src_lang_id=True,
            src_lang_id=0,
        )
        ...
        self.indexed_monolingual_dataset_2 = MonolingualDataset(
            tokens_2,
            sizes=[tokens_2.size(-1)],
            src_vocab=d,
            add_src_lang_id=True,
    ...
    def test_indexed_multi_corpus_sampled_dataset(self):
        class RoundRobinSampler:
            def __init__(self, idx=0) -> None:
                self.idx = idx

            def __call__(self, x):
                ret = x[self.idx % len(x)]
                self.idx += 1
        ...
        m = MultiCorpusSampledDataset(
            OrderedDict(
                {
                    0: self.indexed_monolingual_dataset_1,
                    1: self.indexed_monolingual_dataset_2,
                }
            ),
            sampling_func=RoundRobinSampler(0),
        )
        m.ordered_indices()
        assert (
            m.collater([m[0], m[1]])["net_input"]["src_lang_ids"].tolist()
            == [self.indexed_monolingual_dataset_1.src_lang_id] * 2
        )
        assert (
            m.collater([m[0], m[1]])["net_input"]["src_lang_ids"].tolist()
            == [self.indexed_monolingual_dataset_2.src_lang_id] * 2
        )
        assert (
            m.collater([m[0], m[1]])["net_input"]["src_lang_ids"].tolist()
            == [self.indexed_monolingual_dataset_1.src_lang_id] * 2
