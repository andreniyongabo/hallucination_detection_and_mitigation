...
class TestCoder(unittest.TestCase):
    def test_coder_can_io(self):
        ...
        with NamedTemporaryFile() as tmp_fp:
            ...
            self.assertDictEqual(coder.table, other_coder.table)
...
class TestHuffmanDataset(unittest.TestCase):
    ...
    def test_coder_encode_same_dictionary(self):
        data = make_data()
        builder = make_code_builder(data)
        coder = builder.build_code()
        dico = coder.to_dictionary()

        with TemporaryDirectory() as dirname:
            prefix = os.path.join(dirname, "test")
            build_dataset(prefix, data, coder)

            dataset = HuffmanMMapIndexedDataset(prefix)
            for idx, sentence in enumerate(data):
                dico_encode = dico.encode_line(" ".join(sentence))
                self.assertTrue(torch.equal(dico_encode, dataset[idx].int()))
