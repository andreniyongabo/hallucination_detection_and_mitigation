...
def create_dummy_data(
    data_dir, num_examples=40, maxlen=20, alignment=False, languages=None
):
...
def create_laser_data_and_config_json(data_dir):
    ...
    config_json = {}
    config_train_json = []
    src_vocab = "nllb/test_data/vocab_test_data/sentencepiece.50k.cvocab"
    tgt_vocab = "nllb/test_data/vocab_test_data/sentencepiece.50k.cvocab"
...
def eval_lm_main(
    data_dir, extra_flags=None, path=None, task="language_modeling", world_size=1
):
    if extra_flags is None:
        extra_flags = []
    if path is None:
    ...
    distributed_utils.call_main(cfg, eval_lm.main)
