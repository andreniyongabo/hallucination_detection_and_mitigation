...
def eval_dataset(cfg: DictConfig, eval_split, task, models, start_time):
    ...
    epoch_batch_itr = task.eval_lm_dataloader(
        dataset=dataset,
    ...
    dummy_batch = epoch_batch_itr.first_batch
    itr = epoch_batch_itr.next_epoch_itr(shuffle=False)
    ...
    results = eval_lm(
        ...
        device=torch.device("cuda") if cfg.distributed_training.cpu_offload else None,
    ...
    if isinstance(cfg.common_eval.results_path, str):
...
def main(cfg: DictConfig, **unused_kwargs):
    ...
    if load_sharded:
        assert not is_moe, f"MoE + Loading Sharded State not implemented"
    ...
    for model in models:
        ...
        if (
            ...
            and not cfg.distributed_training.cpu_offload
        ):
            model.cuda()
    ...
    if cfg.dataset.combine_valid_subsets:
        ...
        task.load_dataset(cfg.dataset.gen_subset, combine=True)
    else:
        eval_splits = []
        for valid_sub_split in cfg.dataset.valid_subset.split(","):
            task.load_dataset(valid_sub_split, combine=False)
            eval_splits.append(valid_sub_split)
    ...
    if isinstance(cfg.common_eval.results_path, str):
        if not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0:
            Path(cfg.common_eval.results_path).parent.mkdir(parents=True, exist_ok=True)
            save_json(all_split_results, cfg.common_eval.results_path)
            logger.info(
                "Evaluation results saved to {}".format(cfg.common_eval.results_path)
            )
