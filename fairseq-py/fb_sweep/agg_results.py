...
def make_sweep_table(
    ...
    if keep_cols is not None:
        if isinstance(keep_cols, str):
            keep_cols = (keep_cols,)
        found_keep_cols = [x for x in keep_cols if x in df.columns]
        not_present_cols = [x for x in keep_cols if x not in found_keep_cols]
        if not_present_cols:
            print(f"the following keep-cols were not found: {not_present_cols}")
        df = df[found_keep_cols]

    if "wps" in df.columns:
        df["wps"] = df["wps"].astype(int)
