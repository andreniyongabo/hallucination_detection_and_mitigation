...
def get_grid(args):
    ...
    grids = [
        ...
        hyperparam("--warmup-updates", 4000, save_dir_key=lambda val: f"wu{val}"),
        ...
        # hyperparam("--log-interval", 1000 if not args.local else 10),
    ...
    # mixture of experts (moe) params
    grids.append(hyperparam('--moe-expert-count', args.num_nodes * args.num_gpus * args.experts_per_gpu, save_dir_key=lambda val: f"moe_experts{val}"))
    grids.append(hyperparam("--criterion", "moe_label_smoothed_cross_entropy"))
    grids.append(hyperparam('--moe-gate-loss-wt', [0.01], save_dir_key=lambda val: f'moe_w{val}'))
    ...
    grids.append(hyperparam('--moe-normalize-gate-prob-before-dropping', [False], binary_flag=True, save_dir_key=lambda val: 'norm_b'))
