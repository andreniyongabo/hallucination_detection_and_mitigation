...
for lang in ["amh"]:
    ...
    with open(job_file, "w") as job_out:
        print(
            f"""#!/bin/bash
#SBATCH --job-name=create_index_map.{src}-{tgt}
#SBATCH --output={output_dir}/logs/create_index_map.{src}-{tgt}.%A%a.out
#SBATCH --error={output_dir}/logs/create_index_map.{src}-{tgt}.%A%a.err
...
python examples/nllb/low_resource/ngram_mining/create_index.py \
  --input-path /large_experiments/mmt/ngram_mining/monolingual/{src}_txt/african20/${{SLURM_ARRAY_TASK_ID}}.tok \
  --step map \
  --output-path {output_dir}/${{SLURM_ARRAY_TASK_ID}} \
