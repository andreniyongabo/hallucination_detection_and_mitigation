...
def read_alignments(fin):
    ...
    #    I know recos files have entries missing, but
    #    I want to be consistent with his scores
    return alignments
...
def print_all_scores(b_offset,a_b_costs,alignment_types,file=sys.stdout):
    for a in range(len(b_offset)):
        for al_type in range(len(a_b_costs)):
            for b in range(len(a_b_costs[al_type][a])):
                offset = b_offset[a]
                x,y = ab2xy_w_offset(a, b, b_offset)
                xw,yw = alignment_types[al_type]
                x_list = []
                y_list = []
                for xi in range(xw):
                    x_list.append(x+xi-(xw-1))
                for yi in range(yw):
                    y_list.append(y+yi-(yw-1))
                cost = a_b_costs[al_type][a][b] / xw / yw
                if cost < 9999:
                    print('x %s, y %s, cost %f, ab2xy_w_offset(%d,%d,%d)' % (x_list, y_list,cost, al_type, a, b))


class DeletionKnob(object):
    ##################   I want a knob that makes sense and generalizes across langs/domains/etc...
    ###                     think like a (log) volume for an amplifier
    ###   Linear makes no sense:
    ###         - the smalles value in res is 0.4, then turning the knob from 0.0-0.4 does nothing
    ###         - dito is largest value is 0.8...
    ###   So map it to something that does make sense... how about percentile.
    ###   Most of this code is just making sure that the ends work, even if
    ###           we don't get a full range out of our sample.
    ###
    ...
    def debug_plot(self, del_percentile_frac=None, suptitle=None, axs0=None, axs1=None):
        ...
        xvals = np.linspace(self.res_min, self.res_max, len(self.cdf))
        axs1.plot(xvals, self.cdf, label='CDF (estimated from histogram)')
        axs1.plot(self.y, self.x, '.--', label='CDF points to linearly interpolate')
        axs1.set_ylabel('deletion percentile fraction')
        axs1.set_xlabel('deletion penalty')

        width = 0.7 * (self.bin_edges[1] - self.bin_edges[0])
        center = (self.bin_edges[:-1] + self.bin_edges[1:]) / 2
        axs0.bar(center, self.hist, align='center', width=width, label='histogram of (sampled) 1-1 alignment costs')
        if del_percentile_frac:
            del_pen = self.percentile_frac_to_del_penalty(del_percentile_frac)
            axs1.hlines(y=del_percentile_frac,
                        xmin=self.bin_edges[0],
                        xmax=self.bin_edges[-1], color='g',
                        label='desired deletion percentile fraction =  %.3f' % del_percentile_frac)
            axs1.vlines(x=del_pen, ymin=0, ymax=1, color='r', label='resulting deletion penalty = %.3f' % del_pen)
            axs0.vlines(x=del_pen, ymin=0, ymax=max(self.hist), color='r')
...
def process_scores(scores, alignments):
    # floating point sometimes gives negavite numbers,
    ...
    for ii, (x_algn, y_algn) in enumerate(alignments):
        ...
        #    (still normalized witn random vectors, though)
        else:
...
def append_slant(path, xwidth, ywidth):  # for alignment_to_search_path
    """
    Append quantized approximation to a straigt line 
    ...
    for ii in range(1, NN + 1):
        ...
        # And floating point makes this inherently unstable...
...
def alignment_to_search_path(algn):
    ...
    To make things consistent and to improve the probability of recovering 
       from search errors, we search an (approximatly) straight line
       through a block of deletions. We do the same through a many-many 
       alignment, even though we currently don't refin a many-many alignment...
    """
    path = [(0, 0), ]
    xdel, ydel = 0, 0
    ydel = 0
...
def compute_norms(vecs0, vecs1, num_samples, overlaps_to_use=None):
    ...
    if overlaps_to_use is not None:
        if overlaps_to_use > overlaps1:
            raise Exception('cannot use more overlaps than provided')  # TODO make idiot proof
    else:
    ...
    if size1 and samps_per_overlap:
        # sample other size (from all overlaps) to comare to this side
...
def vecalign(vecs0,
    ...
    for depth in upsample_depths:
        ...
        akey = 'final_alignments' if depth == 0 else 'alignments'  # for debugging, avoid overwriting stack[depth]['alignments']
    ...
    for key in runtimes:
        if runtimes[key] > 5e-4:
            logger.info(key + ' took ' + '.' * (max_key_str_len + 5 - len(key)) + ('%.3fs' % runtimes[key]).rjust(7))
