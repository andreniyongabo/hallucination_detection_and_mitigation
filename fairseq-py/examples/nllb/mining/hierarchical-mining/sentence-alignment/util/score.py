...
"""
Code computes lax and strict precision and recall, following (and based on code from) 
   Bluealign (https://github.com/rsennrich/Bleualign) 

TODO bluealign check license...
...
def _precision(goldalign, testalign):
    ...
    for (test_src, test_target) in testalign:
        ...
        if (test_src, test_target) in goldalign:
            ...
            #   see if there is also partial verlap on the gold/test target
...
def main():
    ...
    gold_list = [pickle.load(open(x, 'rb')) for x in args.gold]
    test_list = [pickle.load(open(x, 'rb')) for x in args.test]
