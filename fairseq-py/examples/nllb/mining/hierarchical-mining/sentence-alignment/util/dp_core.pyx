...
# bounds checking here does not seem to be a bottelneck here
#@cython.boundscheck(False) # turn off bounds-checking for entire function
#@cython.wraparound(False)  # turn off negative index wrapping for entire function
def make_dense_costs(np.ndarray[float, ndim=3] vecs0,  # intput
...
def dense_dp(np.ndarray[float, ndim=2] alignment_cost,
             float pen):
    """
    ...
    # initilize the all c-direction deletion path
    for c in range(cmax):
        csum[0, c] = c * pen
        bp[0, c] = 1

    # initilize the all r-direction deletion path
    for r in range(rmax):
        csum[r, 0] = r * pen
        bp[r, 0] = 2
    ...
    # Initial cost is 0.0
    csum[0, 0] = 0.0  # noop
    bp[0, 0] = 4  # avoid some crazy value from unitilized memory
...
############ actually used for normalization, not a path
...
# Division by 0 checking has minimal speed impact but it disabling 
#      it removes the yellow line in the html....
@cython.boundscheck(False)  # turn off bounds-checking for entire function
@cython.wraparound(False)  # turn off negative index wrapping for entire function
...
def make_sparse_costs(np.ndarray[float, ndim=3] vecs0,  # intput: num aligns X num sents X dim
    ...
    Basically, we take the feature matrix, rotate it 45 degress, 
       and compute a "wavey" matrix for the features. 
        ...
        for b_idx, bb2 in enumerate(range(bb - width_over2, bb + width_over2)):
            ...
            for ii_align in range(num_alignments):
                ...
                if 0 <= xx < xsize and 0 <= yy < ysize:
                    ...
                    for merge_point in (range(x_offset-1)):
                         #if xx+merge_point in do_not_merge0:
                         if xx+merge_point-(x_offset-2) in do_not_merge0:
                             #print("penalizing ({},{}) type ({},{}) x{} a_b_feats({},{},{})".format(xx-(x_offset-1), yy-(y_offset-1), x_offset, y_offset, xx+merge_point-(x_offset-2), ii_align, a_idx, b_idx))
                             feat += 1000.0 * x_offset * y_offset
                    for merge_point in (range(y_offset-1)):
                         #if yy+merge_point in do_not_merge1:
                         if yy+merge_point-(y_offset-2) in do_not_merge1:
                             #print("penalizing ({},{}) type ({},{}) y{} a_b_feats({},{},{})".format(xx-(x_offset-1), yy-(y_offset-1), x_offset, y_offset, yy+merge_point-(y_offset-2), ii_align, a_idx, b_idx))
                             feat += 1000.0 * x_offset * y_offset

                else:
                    feat = inf

                #print("assigning score ({},{}) type ({},{}) a_b_feats({},{},{}) = {}".format(xx-(x_offset-1), yy-(y_offset-1), x_offset, y_offset, ii_align, a_idx, b_idx, feat))
...
def sparse_dp(np.ndarray[float, ndim=3] a_b_costs,
    ...
    As in the simpler full-DP case, 
       we compute cumulitive costs and backpointers on NODES, 
       and there are COSTS associated with moving between them.

    This means the size of the NODES  +1,+1 larger (in x,y) than the COSTS.
    
    SO...  the size of a_b_csum, a_b_xp, a_b_yp are all 
       one larger in x and y compared to the costs
 
    BUT WAIT, THERE'S MORE....

    We are working in a projected "space" (of sorts, definition?)

    In order to save memory (and time, vs a sparse matrix 
       with hashes to look up values), we are working in a "space where:
             a = x + y
             b = x - y

    So the +1,+1 that are o.-so-condusive to off-by-one errors in x,y 
       become a +2,+0 in a,b, leading to off-by-2 errors :)
    ...
    #    origional b_offset, since it should be -width_over2
    ...
    cdef np.ndarray[double, ndim=2] a_b_csum = np.zeros((a_in_size + 2, b_in_size),
                                                        dtype=np.float64) + np.inf  # error cumulitive sum
    ...
    # increasing in a is the same as going along diags in x/y, so DP order works
            ...
            # edge case: all deletions in x-direction
                ...
                for ii_align in range(num_alignments):
                    ...
                    if 0 <= xx_in_cost < x_in_size and 0 <= yy_in_cost < y_in_size and 0 <= xx_out_prev < x_out_size and 0 <= yy_out_prev < y_out_size:
                        #aa_in_cost, bb_in_cost = xy2ab_w_offset(xx_in_cost, yy_in_cost, b_offset_in)  
                        aa_in_cost = xx_in_cost + yy_in_cost
                        bb_in_cost = yy_in_cost - b_offset_in[aa_in_cost]

                        #aa_out_prev, bb_out_prev = xy2ab_w_offset(xx_out_prev, yy_out_prev, b_offset_out)
