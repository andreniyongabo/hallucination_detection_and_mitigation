...
from examples.nllb.mining.monolingual.monolingual_line_processor import (
    FilterConfig,
    LIDConfig,
    MonolingualProcessResult,
    SplitNormalizeFilterLID,
)
from examples.nllb.mining.monolingual.monolingual_sort_dedup import DedupeWithMergeSort
from examples.nllb.mining.monolingual.utils.predict_script import find_lang_script
from examples.nllb.mining.monolingual.utils.sentence_split import map_lang
from examples.nllb.mining.nllb_lib import Launcher, utils
...
class MonoLingualConfig:
    ...
    # corpus migth be empty if corpus_filter is empty. This uses string.Template
...
async def launch_preprocessor(
    ...
    with importlib.resources.path(
        "examples.nllb.mining.monolingual", config.language_script_filename
    ) as path:
        lang_script = find_lang_script(lang, path)

    with importlib.resources.path(
        "examples.nllb.mining.monolingual", config.split_language_equivalences_filename
    ) as path:
        splitter_lang = map_lang(lang, path)

    assert lang_script, f"couldn't find {lang} script in {config.language_script_file}"
    ...
    try:
        ...
        logger.error(
            f"could not clean up the temp shards in {tmp_dir}. Do it manually.",
            exc_info=e,
        )
