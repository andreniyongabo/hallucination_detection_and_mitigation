...
def main():
    ...
    parser.add_argument(
        "--threshold-character", type=str, default="]", help="Threshold character"
    )
    parser.add_argument(
        "--histogram-threshold", type=float, default=0.0, help="threshold"
    )
    parser.add_argument(
        "--histograms",
        type=str,
        default="/checkpoint/edunov/cc60_multilingual/clean_hists/",
        help="Path to histograms",
    )
    parser.add_argument(
        "--show-score",
        action="store_true",
        help="display the accepted characters percentage for each line",
    )
