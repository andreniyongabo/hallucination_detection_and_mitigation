...
from examples.nllb.mining.nllb_lib import nllb_module
from examples.nllb.mining.nllb_lib.utils import bash_pipefail

logger = logging.getLogger("dedup_module")


class DedupeWithMergeSort(nllb_module.NLLBModule):
    """
    this assumes that the input files in config.shards are already sorted
    """

    def requirements(self) -> nllb_module.DistributedRequirements:
