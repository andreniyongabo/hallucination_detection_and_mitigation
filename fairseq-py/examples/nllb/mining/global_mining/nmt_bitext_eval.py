...
import hydra
from modules.evaluation.generate_multi_bleu_detok_module import (
...
from examples.nllb.mining.global_mining.modules.train_fairseq_module import (
    TrainFairseqModule,
)
...
class NMTBitextEval:
    ...
    async def run(self):
        ...
        src_lang_file_after_split: Path = (
            Path(self.config.bin_dir)
            / f"lang1_{self.config.src_lang}_split_tsv_{max_tsv_lines_in_millions}M_lines_{bitext_min_score_threshold}_threshold_input_{self.input_file_mined_data_tsv.stem}"
        )
        tgt_lang_file_after_split: Path = (
            Path(self.config.bin_dir)
            / f"lang2_{self.config.tgt_lang}_split_tsv_{max_tsv_lines_in_millions}M_lines_{bitext_min_score_threshold}_threshold_input_{self.input_file_mined_data_tsv.stem}"
        )
