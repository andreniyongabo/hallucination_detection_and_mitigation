...
# ported from the mlenc repo for parity
...
class MLEncTextEncoder(EncodeToNPY):
    """
    a text encoder is responsible for encoding sentences and writing them to an output file.
    This is the equivalent to the script in embed.py in mlenc without the moses file cleaning. So this
    1. applies SPM, 2. runs the laser2 encoder hardcoded in this file
    """
    ...
    def __init__(
        ...
        spm_model, spm_vocab = self._default_spm(
            encoder_model=encoder_model,
            spm_vocab=spm_vocab,
            spm_model=spm_model,
            fallback_spm=fallback_spm,
        )
    ...
    def _default_spm(
        ...
        if spm_model and spm_vocab:
            return (spm_model, spm_vocab)

        if spm_model and not spm_vocab:
            return (spm_model, str(Path(spm_model).with_suffix(SPM_VOCAB_SUFFIX)))

        logger.warning(
            "spm model or vocab not specified. Defaulting to: <checkpoint>.[spm,cvocab]"
        )
        encoder_path = Path(encoder_model)
        spm_model = encoder_path.with_suffix(SPM_MODEL_SUFFIX)
        spm_vocab = encoder_path.with_suffix(SPM_VOCAB_SUFFIX)
        if spm_model.exists() and spm_vocab.exists():
            spm_model, spm_vocab = str(spm_model), str(spm_vocab)
        else:
            logger.warning(
                "couldn't find <checkpoint>.[spm,cvocab]. Defaulting to fallback spm"
            )
            base = Path(fallback_spm)
            spm_model, spm_vocab = str(base.with_suffix(SPM_MODEL_SUFFIX)), str(
                base.with_suffix(SPM_VOCAB_SUFFIX)
            )
        return spm_model, spm_vocab
