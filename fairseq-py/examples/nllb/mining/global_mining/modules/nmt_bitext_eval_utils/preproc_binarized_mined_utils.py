...
from examples.nllb.mining.global_mining.modules.evaluation.generate_multi_bleu_detok_module import (
    GenerateMultiBleuDetokConfig,
)
from examples.nllb.mining.global_mining.modules.preprocess.line_processor import (
    LineProcessorModule,
)
from examples.nllb.mining.global_mining.modules.preprocess.moses_cli_module import (
    MosesPreprocessConfig,
    MosesPreprocessModule,
)
from examples.nllb.mining.global_mining.modules.preprocess.train_spm import (
    TrainSpmConfig,
    TrainSpmModule,
)
...
class ValidAndTestDataDirConfig:
    ...
    path: Path = "/large_experiments/nllb/mmt/mining/data/flores101/flores101.{split}.{lang}"  # split/lang will be replaced
...
class NMTBitextEvalConfig:
    ...
    eval_minimum_epoch: int = 48  # must be <= eval_maximum_epoch
...
def split_TSV(
    ...
    ) as lang2_out, nllb_lib.utils.open(input_file_mined_data_tsv) as filep:
...
async def process_lang(
    ...
    nllb_lib.utils.symlink(symlinked_dict_file, dict_file)
    ...
    if (
        ...
        nllb_lib.utils.symlink(symlinked_model_file, model_file)
...
# TODO [future PR] change into a module and integrate moses filter step into nmt_bitext_eval script
@dataclass
