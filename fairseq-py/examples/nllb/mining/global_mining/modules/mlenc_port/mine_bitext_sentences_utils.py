...
class Alignments:
    ...
    def __init__(self, scores: np.ndarray, src_idx: np.ndarray, trg_idx: np.ndarray):
        ...
        # TODO: the mlenc mine_indexes module saves this as "trg", but we are using
        # "tgt" everywhere else (especially in our config files)
        # This should be harmonized as soon as the port for mine_indexes is merged
        ...
        with texts_tsv_cm as texts_tsv, meta_tsv_cm as meta_tsv:
            ...
            for score, i_src, i_tgt in zip(self.scores, self.src_idx, self.tgt_idx):
                ...
                if i_src not in self.src_texts or i_tgt not in self.tgt_texts:
                    logger.log(f"Could not find bitext ({i_src, i_tgt}) in text files")
                    continue
