...
def compute_distances(
    ...
    # In the original mlenc code, the embeddings are read in mmap mode,
    # but later copied into memory, so we're directly reading into memory
...
def save_to_disk(
    ...
    dists_out_filename = f"{out_basename}{DISTANCES_FILE_SUFFIX}.npy"
    with open(dists_out_filename, mode="wb") as file:
        if as_fp16:
            np.save(file, distances.astype(np.float16))
        else:
            np.save(file, distances.astype(np.float32))

    indices_out_filename = f"{out_basename}{INDICES_FILE_SUFFIX}.npy"
    with open(indices_out_filename, mode="wb") as file:
