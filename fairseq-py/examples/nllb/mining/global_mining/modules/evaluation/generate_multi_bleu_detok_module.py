...
class GenerateMultiBleuDetokModule(NLLBModule):
    """
...
def multi_bleu_detok_call(
    ...
    with importlib.resources.path(
        "examples.nllb.mining.global_mining.modules.nmt_bitext_eval_utils",
        "multi-bleu-detok.perl",
    ) as path_to_multi_bleu_detok_perl:
        output_ref_file = process_output_ref_hyp_file(
            fairseq_generate_output_file, "ref"
        )
        output_hyp_file = process_output_ref_hyp_file(
            fairseq_generate_output_file, "hyp"
        )

        multi_bleu_detok_command = [
            "perl",
            path_to_multi_bleu_detok_perl,
            output_ref_file,
        ]
