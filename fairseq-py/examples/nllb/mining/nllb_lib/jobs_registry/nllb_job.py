...
class NonIdentifiableJobType(Exception):
    ...
    Job types are specified in JobType Enum in nllb_job.py (array or singular)
    """
...
################################################################################
#  Abstract NLLBJob - A concrete subclass can be created for each new launcher type
#  Right now there's only one launcher: Submitit; the Concrete implementation for NLLBJob is SubmititJob
################################################################################
class NLLBJob(ABC):
    """
    Attributes:
