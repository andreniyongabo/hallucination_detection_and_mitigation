...
from examples.nllb.mining.nllb_lib.jobs_registry.submitit_slurm_job import SubmititJob
from examples.nllb.mining.nllb_lib.nllb_module import (
    DistributedRequirements,
    LocalOnlyRequirements,
)
from examples.nllb.mining.nllb_lib.utils import sha_key
from examples.nllb.modeling.utils import awaitable_job

if tp.TYPE_CHECKING:
    from examples.nllb.mining.nllb_lib import NLLBModule

logger = logging.getLogger("nllb.launcher")
...
class SubmititLauncher(Launcher):
    ...
    def _instantiate_and_register_submitit_job(
        self, submitit_job: submitit.Job, module: "NLLBModule"
    ):
        """
        Takes parameters: submitit_job object and module
        Instantiates submitit_job as an NLLBJob (specifically as a SubmititJob)
        ...
        logger.debug(nllb_job.get_job_info_log())
