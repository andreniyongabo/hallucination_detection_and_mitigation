...
EXPERIMENT_NAME="2021-09-18-00-21-goal124-baseline"
...
prepare_data_3_small () {
    UNSHUFFLED_SMALL_TRAIN_FILE="$TRAIN_DIR/small.unshuf.txt"
    > $UNSHUFFLED_SMALL_TRAIN_FILE
    UNSHUFFLED_SMALL_VALID_FILE="$VALID_DIR/small.unshuf.txt"
    > $UNSHUFFLED_SMALL_VALID_FILE
    SMALL_TRAIN_FILE="$TRAIN_DIR/small.txt"
    > $SMALL_TRAIN_FILE
    SMALL_VALID_FILE="$VALID_DIR/small.txt"
    > $SMALL_VALID_FILE
    ...
    for i in "${!GOAL124__ISO_639_3_LANGS[@]}"; do
        ...
        cat $TRAIN_FILE | head -n 10000 >> $UNSHUFFLED_SMALL_TRAIN_FILE
        cat $VALID_FILE | head -n 10000 >> $UNSHUFFLED_SMALL_VALID_FILE

    done

    shuf $UNSHUFFLED_SMALL_TRAIN_FILE > $SMALL_TRAIN_FILE
    shuf $UNSHUFFLED_SMALL_VALID_FILE > $SMALL_VALID_FILE
...
train_fasttext_1_small () {
    echo "Training fastText:"
    $FASTTEXT_BIN supervised -input $TRAIN_DIR/small.txt -output $RESULT_FOLDER/model.small.1 \
        -lr 0.5 -minn 2 -maxn 5 -minCount 1 -epoch 1 -dim 16 -loss ova -bucket 5000000 -thread 10
}

train_fasttext_2_small () {
    echo "Training fastText:"
    $FASTTEXT_BIN supervised -input $TRAIN_DIR/small.txt -output $RESULT_FOLDER/model.small.2 \
        -lr 0.5 -minn 2 -maxn 5 -minCount 1 -epoch 3 -dim 16 -loss ova -bucket 5000000 -thread 10
}

train_fasttext_3_small () {
    echo "Training fastText:"
    $FASTTEXT_BIN supervised -input $TRAIN_DIR/small.txt -output $RESULT_FOLDER/model.small.3 \
        -lr 0.5 -minn 2 -maxn 5 -minCount 1 -epoch 5 -dim 16 -loss ova -bucket 5000000 -thread 10
}

train_fasttext_4_small () {
    echo "Training fastText:"
    $FASTTEXT_BIN supervised -input $TRAIN_DIR/small.txt -output $RESULT_FOLDER/model.small.4 \
        -lr 0.5 -minn 2 -maxn 5 -minCount 100 -epoch 5 -dim 64 -loss softmax -bucket 10000000 -thread 40
}
...
eval_flores_devtest_fasttext_variants_small () {
    GOLD="$RESULT_FOLDER/flores-devtest.gold"
    TEST_FILE="$TEST_DIR/flores-devtest.txt"
    cat "$TEST_FILE" | cut -f 1 -d" " > $GOLD
    ...
    for i in `seq 1 39`
    do
        RESULT_TXT="$RESULT_FOLDER/result.classifiermetrics-flores-devtest.fasttext.small.$i.txt"

        LID_MODEL="$RESULT_FOLDER/model.small.$i.bin"
        PREDICTIONS="$RESULT_FOLDER/flores-devtest.fasttext.predictions.small.$i"
