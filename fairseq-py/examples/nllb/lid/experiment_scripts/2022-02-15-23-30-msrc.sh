...
EXPERIMENT_NAME="2022-02-15-23-30-msrc"
...
FBSEED_DATA_DIR="/private/home/celebio/fbseed20220214_data"
...
prepare_data_filter_step_1_jpn () {
    ...
    paste $LANG_SCRIPT_PRED_FILE $MONO_CAT_FILE | grep "^Jpan" | cut -f 2 > $CLEANED_TRAIN_FILE
}
...
prepare_flores_filled_dev_data () {
    ...
    for i in "${!NLLB_2022__ISO3[@]}"; do
        ...
        then
            ...
            if [ -f "$FLORES_DEV_FILE" ]; then
                chosen_source="${chosen_source}-flores"
                cat $FLORES_DEV_FILE | awk -v lang="$NLLB_LANG" '{print "__label__"lang " " $0}' >> $CONCAT_FILE
            fi
        fi
...
prepare_flores_filled_devtest_data () {
    ...
    for i in "${!NLLB_2022__ISO3[@]}"; do
        ...
        if [ -z "$chosen_source" ]
        then
            FLORES_DEVTEST_FILE="$FLORES_DEVTEST_DIR/$FLORES_LANG.devtest"
            if [ -f "$FLORES_DEVTEST_FILE" ]; then
                chosen_source="${chosen_source}-flores"
                cat $FLORES_DEVTEST_FILE | awk -v lang="$NLLB_LANG" '{print "__label__"lang " " $0}' >> $CONCAT_FILE
            fi
        fi

        if [ -z "$chosen_source" ]
        then
            FLORES_DEVTEST_FILE="$FLORES_BETA_DEVTEST_DIR/$FLORES_LANG.devtest"
            if [ -f "$FLORES_DEVTEST_FILE" ]; then
                chosen_source="${chosen_source}-flores_beta"
                cat $FLORES_DEVTEST_FILE | awk -v lang="$NLLB_LANG" '{print "__label__"lang " " $0}' >> $CONCAT_FILE
            fi
...
train_fasttext_8_3 () {
    ...
    $FASTTEXT_BIN supervised -input $TRAIN_DIR/all.txt -output $RESULT_FOLDER/model.8.3 \
        -lr 0.8 -minn 2 -maxn 5 -minCount 1000 -epoch 5 -dim 256 -loss softmax -bucket 1000000 -thread 40
}

train_fasttext_8_4 () {
    echo "Training fastText:"
    $FASTTEXT_BIN supervised -input $TRAIN_DIR/all.txt -output $RESULT_FOLDER/model.8.4 \
        -lr 1.0 -minn 2 -maxn 5 -minCount 1000 -epoch 10 -dim 256 -loss softmax -bucket 1000000 -thread 40
}

train_fasttext_8_5 () {
    echo "Training fastText:"
    $FASTTEXT_BIN supervised -input $TRAIN_DIR/all.txt -output $RESULT_FOLDER/model.8.5 \
        -lr 1.0 -minn 2 -maxn 5 -minCount 1 -epoch 10 -dim 256 -loss softmax -bucket 1000000 -thread 40
}
...
train_fasttext_9_1 () {
    ...
    $FASTTEXT_BIN supervised \
        ...
        -output $RESULT_FOLDER/model.9.1 \
...
train_fasttext_9_2 () {
    ...
    $FASTTEXT_BIN supervised \
        ...
        -output $RESULT_FOLDER/model.9.2 \
        -lr 0.8 -minn 2 -maxn 5 -minCount 1 \
        -epoch 5 -dim 256 \
        -loss hs \
        -bucket 5000000 \
        -thread 40
...
group_arabic_predictions_gold_dev () {
    GOLD="$RESULT_FOLDER/flores-filled-dev.gold"
    GOLD_GROUPED="$RESULT_FOLDER/flores-filled-dev.grouped.gold"
    group_langs $GOLD $GOLD_GROUPED

    for MODEL_MAJOR in 8 9
    do
        for i in `seq 1 20`
        do
            RESULT_TXT="$RESULT_FOLDER/result.classifiermetrics-flores-filled-dev.fasttext.$MODEL_MAJOR.$i.grouped.txt"

            PREDICTIONS="$RESULT_FOLDER/flores-filled.dev.fasttext.predictions.$MODEL_MAJOR.$i"
            PREDICTIONS_GROUPED="$RESULT_FOLDER/flores-filled.dev.fasttext.predictions.$MODEL_MAJOR.$i.grouped"

            LID_MODEL="$RESULT_FOLDER/model.$MODEL_MAJOR.$i.bin"

            if [ -s $LID_MODEL ] && [ ! -s $RESULT_TXT ]
            then
                group_langs $PREDICTIONS $PREDICTIONS_GROUPED
                $CLASSIFIER_METRICS --prediction $PREDICTIONS_GROUPED --gold $GOLD_GROUPED > $RESULT_TXT
            fi
...
train_fasttext_8_1
eval_flores_filled_dev_fasttext_variants
eval_flores_filled_devtest_fasttext_variants_8
