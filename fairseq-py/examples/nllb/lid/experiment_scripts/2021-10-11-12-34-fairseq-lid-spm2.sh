...
EXPERIMENT_NAME="2021-10-11-12-34-fairseq-lid-spm2"
...
prepare_spm_data_1 () {
    ...
    for i in "${!GOAL124__ISO_639_3_LANGS[@]}"; do
        ...
        cat $TRAIN_FILE | cut -f2- -d" " | head -n $SPM_LIMIT_SENTENCE_NB > $SPM_TRAIN_FILE
    done
...
EOM
  ...
  chmod +x $TRAIN_SH
      ...
      # --partition=nllb --comment "LID" \

  sbatch -J "LID-spm-cleaner" \
      --partition=learnaccel --comment "LID-spm-cleaner" \
