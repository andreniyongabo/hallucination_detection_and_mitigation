...
EXPERIMENT_NAME="2021-12-27-14-48-multifilter6-ndup"
...
prepare_data_combine() {
    ...
    do
        ...
        LC_COLLATE=C cat $MONO_CAT_FILE | LC_COLLATE=C sort | LC_COLLATE=C uniq | LC_COLLATE=C shuf > $MONO_SHUF_FILE

        NUMBER_OF_LINES=$(cat $MONO_SHUF_FILE | wc -l)
...
prepare_flores_filled_dev_data () {
    CONCAT_FILE="$VALID_DIR/concat.flores-filled-dev.txt"
    VALID_FINAL_FILE="$VALID_DIR/flores-filled-dev.txt"
    > $CONCAT_FILE
    ...
    do
        ...
        FLORES_FILE="$FLORES_DEV_DIR/${FLORES_LANG}.dev"
    ...
    shuf $CONCAT_FILE > $VALID_FINAL_FILE
    rm $CONCAT_FILE
}
