...
class _Meter(object):
    ...
    def f1_score(self, label=None):
        """Return global recall or for a label"""
        if label:
            label_id = self.f.get_label_id(label)
            f1 = self.m.f1ScoreLabel(label_id)
        else:
            f1 = self.m.f1Score()
    ...
    def precision(self, label=None):
        ...
        if label:
            label_id = self.f.get_label_id(label)
            precision = self.m.precisionLabel(label_id)
        else:
            precision = self.m.precision()
    ...
    def recall(self, label=None):
        ...
        if label:
            label_id = self.f.get_label_id(label)
            recall = self.m.recallLabel(label_id)
        else:
            recall = self.m.recall()
