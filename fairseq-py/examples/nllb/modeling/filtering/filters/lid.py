...
class LidFilter(Filter):
    ...
    def filter_line(
        ...
        # NB: assumes all languages are filtered
        # assert self.src_threshold, f"no threshold for {self.src_lang}"
        # assert self.tgt_threshold, f"no threshold for {self.tgt_lang}"

        # certain corpora are excluded from LID filtering because we trust them
        # more than the LID model 8)
