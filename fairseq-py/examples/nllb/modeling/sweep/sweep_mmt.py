...
def get_grid(args):
    ...
    grids = [
        ...
        hyperparam(
            "--max-source-positions", args.max_pos, save_dir_key=lambda val: f"max_pos{val}"
        ),
    ...
    if args.checkpoint_activations:
        grids.append(
            hyperparam("--checkpoint-activations", binary_flag=True, save_dir_key=lambda val: f"CA"),
        )
    # optimizer
    ...
    if args.encoder_langtok:
        grids.append(
            hyperparam(
                "--encoder-langtok", args.encoder_langtok, save_dir_key=lambda val: f"ent{val}"
            )
        )
...
def added_adapter_transformer() -> List[hyperparam]:
    return [
        hyperparam(
            "--arch", ["added_adapter_transformer"], save_dir_key=lambda val: f"arch{val}"
        )
    ]
