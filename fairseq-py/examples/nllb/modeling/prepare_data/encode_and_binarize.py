...
from examples.nllb.modeling.tokenizer.spm_tokenizer import SPMTokenizer
from examples.nllb.modeling.utils import execute_in_shell

logger = logging.getLogger("encode_and_binarize")
...
def _length_based_filtering(
    ...
    logger.info(f"Length based filtering for {spm_path} {src_lang} {tgt_lang}")
    if not os.path.exists(encoded_outdir):
        os.makedirs(encoded_outdir, exist_ok=True)
    if os.path.exists(
        f"{encoded_outdir}/spm_length_filtered.{src_lang}-{tgt_lang}.{src_lang}"
    ):
        os.remove(
            f"{encoded_outdir}/spm_length_filtered.{src_lang}-{tgt_lang}.{src_lang}"
        )
    if os.path.exists(
        f"{encoded_outdir}/spm_length_filtered.{src_lang}-{tgt_lang}.{tgt_lang}"
    ):
        os.remove(
            f"{encoded_outdir}/spm_length_filtered.{src_lang}-{tgt_lang}.{tgt_lang}"
        )
    command = (
        ...
        f"{src_lang} "
        f"{tgt_lang} "
        f"{encoded_outdir}/spm_length_filtered.{src_lang}-{tgt_lang} "
        "1 "
        "250 "
    )
    execute_in_shell(command)
    return f"{encoded_outdir}/spm_length_filtered.{src_lang}-{tgt_lang}"
...
def _binarize_step(
    ...
    _binarize(
        source,
        target,
        encoded_prefix if tag == "train" or tag == "train_secondary" else None,
        encoded_prefix if tag == "valid" or tag == "sampled_train" else None,
    ...
    file_tag = "train" if tag == "train_secondary" else tag
...
def encode_and_binarize(
    ...
    (encoded_src_path, encoded_tgt_path) = _encode_spm_step(
        ...
        custom_step_name=f"encode_spm.{tag}.{direction}.shard{shard_id}",
    )
    ...
    if encoded_filtered_outdir is not None:
        encoded_prefix = _length_based_filtering(
            ...
            output_dir=output_dir,
            custom_step_name=f"length_filtered.{tag}.{direction}.shard{shard_id}",
        )
    ...
    binarize_results = _binarize_step(
        ...
        custom_step_name=f"binarize.{tag}.{direction}.shard{shard_id}",
    )
