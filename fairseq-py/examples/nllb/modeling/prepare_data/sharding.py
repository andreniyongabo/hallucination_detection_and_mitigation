...
logger = logging.getLogger("prepare_data_sharding")


def get_all_num_shards(
    train_counts_map: Dict[str, int],
    secondary_train_counts_map: Dict[str, int],
    data_config: data_types.DataConfig,
):
    total_train_size = sum(train_counts_map.values()) + sum(
        secondary_train_counts_map.values()
    )
    directions = list(
    ...
    all_num_secondary_shards = {
        direction: get_num_shards(
            num_lines=secondary_train_counts_map.get(direction, 0),
            smallest_shard=data_config.binarization_config.smallest_shard,
            max_num_shards=max_num_shards,
        )
        for direction in directions
    }
    return all_num_shards, all_num_secondary_shards
