...
logger = logging.getLogger("prepare_vocab")
...
async def get_vocab(
    ...
    train_corpora: Dict[str, data_types.ParallelDataset],
    secondary_train_corpora: Dict[str, data_types.ParallelDataset],
    ...
    if pretrained_source and pretrained_target:
        ...
        if secondary_train_corpora:
            for direction, parallel_dataset in secondary_train_corpora.items():
                src_lang_files[direction.split("-")[0]].append(parallel_dataset.source)
                tgt_lang_files[direction.split("-")[1]].append(parallel_dataset.target)
