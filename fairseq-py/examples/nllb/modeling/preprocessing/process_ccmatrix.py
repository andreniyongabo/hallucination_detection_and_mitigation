...
def main(args):
    ...
    target_lines = []
    total_counter = 0
    with gzip.open(f"{args.basepath}/{args.src}-{args.tgt}.bitextf.tsv.gz", "rt", encoding="utf8") as f:
    ...
    print(f"Remaining Lines = {len(source_lines)} out of {total_counter} original lines. Percentage kept = {float(len(source_lines))/total_counter}")
