...
for lang in langs:
    if not f"en-{lang}" in directions and not f"{lang}-en" in directions:
        ...
        cc100_lang = flores_to_cc100_langs[lang] if lang in flores_to_cc100_langs else lang
        if not f"en-{cc100_lang}" in directions and not f"{cc100_lang}-en" in directions:
...
for lang in langs:
    if lang in in_old_m2m100 and not lang in in_holger_mined:
        print(lang)
        num_old += 1
    elif lang in in_old_m2m100 and lang in in_holger_mined and flores_to_iso_langs[lang] in worse_in_new_mined:
        print(lang)
        num_old += 1
