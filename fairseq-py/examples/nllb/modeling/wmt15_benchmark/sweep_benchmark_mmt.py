...
def get_grid(args):
    ...
    grids = [
        ...
        hyperparam("--update-freq", [args.update_freq], save_dir_key=lambda val: f"uf{val}"), # args.update_freq),
        ...
        hyperparam("--arch", ["transformer_wmt_en_de"], save_dir_key=lambda val: f"arch{val}"),
        ...
        hyperparam("--warmup-updates", [8000],),
        hyperparam("--lr", args.lr, save_dir_key=lambda val: f"lr{val}"),
        hyperparam("--stop-min-lr", 1e-9),
        hyperparam("--clip-norm", 0.0,),
        hyperparam("--dropout", args.dropout,),
        hyperparam("--weight-decay", 0.0,),
        hyperparam("--criterion", "label_smoothed_cross_entropy" if not args.moe else "moe_label_smoothed_cross_entropy"),
        hyperparam("--label-smoothing", 0.1,),
        hyperparam("--max-tokens", args.max_tokens, save_dir_key=lambda val: f"maxtok{val}"),
        hyperparam("--seed", [2],),
    ...
    if args.moe:
        ...
        grids.extend([
            ...
            hyperparam('--moe-normalize-gate-prob-before-dropping', [False], binary_flag=True, save_dir_key=lambda val: 'norm_b'),
            hyperparam('--moe-gating-use-fp32'),
            hyperparam('--encoder-moe-freq', args.encoder_moe_freq, save_dir_key=lambda val: f"emq{val}"),
            hyperparam('--decoder-moe-freq', args.decoder_moe_freq, save_dir_key=lambda val: f"dmq{val}"),
