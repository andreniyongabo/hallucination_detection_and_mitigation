...
LANGS = ['cs', 'de', 'es', 'et', 'fi', 'fr', 'gu', 'hi', 'kk', 'lt', 'lv', 'ro', 'ru', 'tr', 'zh']
...
def get_bleus(type="valid"):
    ...
    for lang in LANGS:
        ...
        for updates in UPDATES_LST:
            try:
                fname = list(glob.glob(os.path.join(model_dir, "gen_output", f"{src}-{tgt}_checkpoint_*_{updates}_{type}", "bleu.results")))[0]
...
def print_bleus(type):
    print(f"{type}\t" + "\t".join([str(u) for u in UPDATES_LST]) + "\tBEST BLEU\tBEST_CHECKPOINT")

    bleus = valid_bleus if type == "valid" else test_bleus
    for lang in LANGS:
        src = lang
        tgt = "en"
        out_line = f"{src}-{tgt}\t" + "\t".join([f"{bleus[lang][u]}" for u in UPDATES_LST])
        best_updates = max(bleus[lang], key=bleus[lang].get)
        out_line += f"\t{bleus[lang][best_updates]}\t{best_updates}"
        #print(out_line)
        if type == "valid":
            out_line = f"{src}-{tgt}-valid-ppl\t" + "\t".join([f"{valid_ppls[lang][u]}" for u in UPDATES_LST])
