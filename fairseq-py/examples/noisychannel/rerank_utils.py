...
def lm_scoring(
    ...
    if bpe_status == "no bpe":
        ...
        # spm instead of bpe
        # bpe_src_param = [cur_lm_bpe_code, rescore_file+target_lang, rescore_bpe+target_lang]
        subprocess.call(f"python scripts/spm_encode.py --model {cur_lm_bpe_code} --inputs {rescore_file+target_lang} --outputs {rescore_bpe+target_lang}", shell=True)
