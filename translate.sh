...
CUSTOMDIR=$HOME/hallucination_detection_and_mitigation/translations/flores_test/
...
mkdir -p ${CUSTOMDIR}/${src}-${tgt}/
prefix=${CUSTOMDIR}/${src}-${tgt}/output
beam=4
batch_size=1

# for tokenization, if src=="eng" use ${src:0:2}, otherwise use ${src}
cat ${input_file} |  ${NORM_PUNCT} -l ${src:0:2} | python fairseq-py/fairseq_cli/interactive.py ${DATADIR}/data_bin/shard000 \
...
cat ${prefix}.gen_log | grep -P "^D-"| cut -f2 > ${prefix}.sent_score