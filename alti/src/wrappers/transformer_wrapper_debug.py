...
class FairseqTransformerHub(GeneratorHubInterface):
    ...
    def __get_contributions_module(self, layer_inputs, layer_outputs, contrib_type, pre_layer_norm, module_name):
        ...
        if contrib_type == 'l1':
            pieces = transformed_vectors.size(1)//10
            # print(pieces)
            if pieces > 0:
                transformed_vectors_chunk = torch.chunk(transformed_vectors, pieces, dim=1)
                resultant_chunk = torch.chunk(resultant, pieces, dim=1)
                list_chunks = []
                for i in range(pieces):
                    list_chunks.append(-F.pairwise_distance(transformed_vectors_chunk[i], resultant_chunk[i].unsqueeze(2), p=1))
                contributions = torch.cat(list_chunks, dim=1)
            ...
            if pieces > 0:
                ...
                list_chunks = []
                for i in range(pieces):
                    list_chunks.append(-F.pairwise_distance(transformed_vectors_chunk[i], resultant_chunk[i].unsqueeze(2), p=2))
                contributions = torch.cat(list_chunks, dim=1)
