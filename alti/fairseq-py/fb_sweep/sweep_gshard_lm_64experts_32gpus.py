...
def get_grid(args):
    ...
    grid = [
        ...
        hyperparam("--tokens-per-sample", TPS, save_dir_key=lambda val: f"tps{val}"),
        hyperparam(
    ...
    if os.getenv("S3_DIR", False):
        grid.extend(
            [
                hyperparam("--save-async", binary_flag=True),
                hyperparam("--restore-file", "x.pt"),
                hyperparam(
                    "--s3-dir", os.getenv("S3_DIR"), save_dir_key=lambda x: "s3"
                ),
            ]
        )
    return grid
