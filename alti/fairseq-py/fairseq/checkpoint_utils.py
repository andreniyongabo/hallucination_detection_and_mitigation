...
def load_checkpoint(cfg: CheckpointConfig, trainer, **passthrough_args):
    ...
    *passthrough_args* will be passed through to
    ...
    suffix = (
        trainer.checkpoint_suffix if not getattr(cfg, "ignore_suffix", None) else None
    )
    if (
...
def load_checkpoint_to_cpu(
    ...
    if is_moe and os.path.exists(shared_path):
        ...
        expert_state = moe_checkpoint_utils.load_expert_state(paths_to_load)
...
def load_model_ensemble_and_task(
    ...
    for filename in filenames:
        ...
        for shard_idx in range(num_shards):
            ...
            if getattr(cfg.model, "arch", None) == "transformer_lm_megatron":
            ...
            #print ('task1',len(task.target_dictionary))
            if "task_state" in state:
                task.load_state_dict(state["task_state"])
            #print ('task2',len(task.target_dictionary))
