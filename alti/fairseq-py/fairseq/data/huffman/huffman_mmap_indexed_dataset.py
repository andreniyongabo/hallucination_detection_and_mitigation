...
class HuffmanMMapIndexedDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, prefix_path, append_eos=True):
        super().__init__()
        ...
        self._do_init(prefix_path=prefix_path, append_eos=append_eos)

    def __getstate__(self):
        return (self._prefix_path, self._append_eos)

    def __setstate__(self, state):
        prefix_path, append_eos = state
        self._do_init(prefix_path=prefix_path, append_eos=append_eos)

    def _do_init(self, prefix_path, append_eos=True):
        ...
        self._dico = self._coder.to_dictionary()
    ...
    def __getitem__(self, i):
        ...
        tensor = self._dico.encode_line(
            [n.symbol for n in nodes],
            line_tokenizer=lambda l: l,
            add_if_not_exist=False,
            append_eos=self._append_eos,
        )
        # dictionary returns int32, but we need int64
