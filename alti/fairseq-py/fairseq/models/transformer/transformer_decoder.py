...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        if (
            not getattr(args, "adaptive_input", None)
            and getattr(args, "quant_noise_pq", 0) > 0
        ):
        ...
        moe_freq = max(
            getattr(args, "decoder_moe_freq", 0), getattr(args, "moe_freq", 0)
        )
        ...
        self.project_out_dim = (
            ...
            and not getattr(args, "tie_adaptive_weights", None)
            else None
        )

        self.adaptive_softmax = None
        self.output_projection = None
        if getattr(args, "adaptive_softmax_cutoff", None) is not None:
    ...
    def extract_features_scriptable(
        ...
        for idx, layer in enumerate(self.layers):
            prev_output_tokens = (
                prev_output_tokens
                if getattr(self.args, "pass_tokens_transformer_layer", False)
                else None
            )
