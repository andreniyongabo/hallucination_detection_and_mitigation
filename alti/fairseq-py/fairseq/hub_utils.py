...
def from_pretrained(
    ...
    **kwargs,
    ...
    #print('hubutils',len(task.target_dictionary))
    return {
        "args": args,
        "task": task,
        "models": models,
...
class GeneratorHubInterface(nn.Module):
    ...
    def setup_task(self,add_lang_tok=True):
        ...
        if "langs" in self.cfg.task:
            ...
            lang_tokens = [
                ...
                self.to_lang_token(x.strip()) for x in self.cfg.task.langs
            ]

            # for debug purpos
            #:wqprint('addlangtok',add_lang_tok)
            if add_lang_tok:
    ...
    def __init__(
        ...
        self.task = task
        #print ('setup1',len(self.task.target_dictionary))
        self.setup_task(add_lang_tok)
        #print ('setup2',len(self.task.target_dictionary))
