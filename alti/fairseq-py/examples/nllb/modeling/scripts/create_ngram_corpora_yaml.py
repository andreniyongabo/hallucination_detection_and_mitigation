...
def main(args):
    ...
    for direction_directory in direction_directories:
        ...
        if args.use_reverse_direction:
            os.makedirs(os.path.join(args.output_path, reverse_direction), exist_ok=True)
        src_files = glob.glob(f"{direction_directory}/mined.bleu/*.{src}")
        for src_file in src_files:
            corpus = "mined.bleu.30"
            tgt_file = os.path.join(direction_directory, "mined.bleu", f"mined.bleu.30.{src}-{tgt}.{tgt}")
            corpus_name = CORPUS_NAME_MAP.get(corpus, corpus)
            corpus_to_directions[corpus_name].append(direction)

            with open(os.path.join(args.output_path, direction, f"{corpus_name}.yaml"), "w") as yaml_out:
            ...
            if args.use_reverse_direction:
                ...
                with open(os.path.join(args.output_path, reverse_direction, f"{corpus_name}.yaml"), "w") as yaml_out:
                    print(TEMPLATE.format(
                        corpus_name=corpus_name,
