...
def grad2vec(model, modules="all"):
    ...
    if modules == "encoder\emb":
        return torch.cat([p.grad.data.view(-1) for p in model.encoder.parameters() if not p.size()[0] == 64006 ])
    if modules == "decoder":
        return torch.cat([p.grad.data.view(-1) for p in model.decoder.parameters()])
    if modules == "decoder\emb":
        return torch.cat([p.grad.data.view(-1) for p in model.decoder.parameters() if not p.size()[0] == 64006 ])
        
        
def compute_gradients(langdir, task, model, criterion,
                      split='valid_main'):
    model.train()
    zero_grad(model)
    ...
    grads = {"all": grad2vec(model, modules="all") / count,
...
def report(similarities, langs, output_dir, label="Title"):
    ...
    ax = sns.heatmap(M, linewidths=.5, vmin=0.0, vmax=1.)
...
async def main(args) -> None:
    ...
    # Similarities
    similarities_all, similarities_enc, similarities_enc_woemb, similarities_dec, similarities_dec_woemb = {}, {}, {}, {}, {}
