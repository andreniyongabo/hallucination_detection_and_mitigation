...
void Meter::log(
  ...
  metrics_.negative += nlabels_ - labels.size();
  ...
  for (const auto& prediction : predictions) {
    ...
    if (utils::contains(labels, prediction.second)) {
      ...
      labelMetrics_[prediction.second].predictedNegative++;
      metrics_.predictedNegative++;
    }
    labelMetrics_[prediction.second].scoreVsTrue.emplace_back(score, gold);
  }

  for (int32_t label=0; label<nlabels_; label++){
    if (!utils::contains(labels, label)) {
      labelMetrics_[label].negative++;
    }
  }
...
double Meter::fpr(int32_t i) {
  return labelMetrics_[i].fpr();
}
...
void Meter::writeGeneralMetrics(std::ostream& out, int32_t k) const {
  ...
  out << "FPR@" << k << "\t" << metrics_.fpr() << std::endl;
}
