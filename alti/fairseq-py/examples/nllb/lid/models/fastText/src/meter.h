...
class Meter {
  struct Metrics {
    ...
    uint64_t predictedNegative;
    mutable std::vector<std::pair<real, real>> scoreVsTrue;

    Metrics() : gold(0), negative(0), predicted(0), predictedGold(0), predictedNegative(0), scoreVsTrue() {}
    ...
    double fpr() const {
      ...
      return predictedNegative / double(negative);
    }
 ...
 public:
  Meter() = delete;
  explicit Meter(int32_t nlabels, bool falseNegativeLabels)
      : metrics_(),
        nexamples_(0),
        nlabels_(nlabels),
        labelMetrics_(),
