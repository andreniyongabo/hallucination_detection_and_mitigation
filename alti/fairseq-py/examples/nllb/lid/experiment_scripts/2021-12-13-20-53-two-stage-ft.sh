...
EXPERIMENT_NAME="2021-12-13-20-53-two-stage-ft"
...
prepare_data_combine_spec_1 () {
    cat "$TRAIN_DIR/zho.train.mono.txt" "$TRAIN_DIR/yue.train.mono.txt" | shuf > "$TRAIN_DIR/all.spec1.txt"
}

prepare_data_combine_spec_2 () {
    cat "$TRAIN_DIR/bos.train.mono.txt" "$TRAIN_DIR/hrv.train.mono.txt" | shuf > "$TRAIN_DIR/all.spec2.txt"
}

prepare_data_combine_spec_3 () {
    cat "$FLORES_DEV_DIR/zho_simpl.dev" | awk -v lang="zho" '{print "__label__"lang " " $0}' > "$TRAIN_DIR/zho_simpl.flores.txt"
    cat "$FLORES_DEV_DIR/yue.dev" | awk -v lang="yue" '{print "__label__"lang " " $0}' > "$TRAIN_DIR/yue.flores.txt"

    cat "$TRAIN_DIR/zho_simpl.flores.txt" "$TRAIN_DIR/yue.flores.txt" | shuf > "$TRAIN_DIR/all.spec3.txt"

}


train_fasttext_8_1_spec_3 () {
    echo "Training fastText:"
    $FASTTEXT_BIN supervised -input $TRAIN_DIR/all.spec3.txt -output $RESULT_FOLDER/model.8.spec3 \
        -lr 0.8 -minn 2 -maxn 5 -epoch 40 -dim 16 -loss softmax -bucket 1000000 -thread 10
}

train_fasttext_8_1_spec_1 () {
    echo "Training fastText:"
    $FASTTEXT_BIN supervised -input $TRAIN_DIR/all.spec1.txt -output $RESULT_FOLDER/model.8.spec1 \
        -lr 0.8 -minn 2 -maxn 5 -epoch 40 -dim 16 -loss softmax -bucket 1000000 -thread 10
}

train_fasttext_8_1_spec_2 () {
    echo "Training fastText:"
    $FASTTEXT_BIN supervised -input $TRAIN_DIR/all.spec2.txt -output $RESULT_FOLDER/model.8.spec2 \
        -lr 0.5 -minn 1 -maxn 5 -epoch 10 -dim 64 -loss softmax -bucket 1000000 -thread 10
}

train_fasttext_8_1_spec_2_1 () {
    echo "Training fastText:"
    $FASTTEXT_BIN supervised -input $TRAIN_DIR/all.spec2.txt -output $RESULT_FOLDER/model.8.spec2 \
        -lr 0.5 -minn 1 -maxn 5 -epoch 20 -dim 128 -loss softmax -bucket 10000000 -thread 40
}

train_fasttext_8_1_spec_2_2 () {
    echo "Training fastText:"
    $FASTTEXT_BIN supervised -input $TRAIN_DIR/all.spec2.txt -output $RESULT_FOLDER/model.8.spec2 \
        -lr 0.5 -minn 1 -maxn 10 -epoch 20 -dim 128 -loss softmax -bucket 10000000 -thread 60
}

train_fasttext_8_1_spec_2_3 () {
    echo "Training fastText:"
    $FASTTEXT_BIN supervised -input $TRAIN_DIR/all.spec2.txt -output $RESULT_FOLDER/model.8.spec2 \
        -lr 0.5 -minn 1 -maxn 10 -epoch 20 -dim 256 -loss softmax -bucket 10000000 -thread 60
}
...
eval_flores_filled_fasttext_variants_8_spec () {
    TEST_FILE_BIG="$TEST_DIR/flores-filled.txt"

    TEST_FILE="$TEST_DIR/flores-filled.spec1.txt"
    cat $TEST_FILE_BIG | grep "^__label__yue" > "$TEST_DIR/flores-filled.yue.txt"
    cat $TEST_FILE_BIG | grep "^__label__zho" > "$TEST_DIR/flores-filled.zho.txt"
    cat "$TEST_DIR/flores-filled.yue.txt" "$TEST_DIR/flores-filled.zho.txt" | shuf > "$TEST_FILE"

    TEST_FILE="$TEST_DIR/flores-filled.spec2.txt"
    cat $TEST_FILE_BIG | grep "^__label__bos" > "$TEST_DIR/flores-filled.bos.txt"
    cat $TEST_FILE_BIG | grep "^__label__hrv" > "$TEST_DIR/flores-filled.hrv.txt"
    cat "$TEST_DIR/flores-filled.bos.txt" "$TEST_DIR/flores-filled.hrv.txt" | shuf > "$TEST_FILE"


    TEST_FILE="$TEST_DIR/flores-filled.spec3.txt"
    cat $TEST_FILE_BIG | grep "^__label__yue" > "$TEST_DIR/flores-filled.yue.txt"
    cat $TEST_FILE_BIG | grep "^__label__zho" > "$TEST_DIR/flores-filled.zho.txt"
    cat "$TEST_DIR/flores-filled.yue.txt" "$TEST_DIR/flores-filled.zho.txt" | shuf > "$TEST_FILE"
    ...
    for i in `seq 3 9`
    do
        RESULT_TXT="$RESULT_FOLDER/result.classifiermetrics-flores-filled.fasttext.8.spec$i.txt"

        LID_MODEL="$RESULT_FOLDER/model.8.spec$i.bin"

        if [ -s $LID_MODEL ]
        then
            TEST_FILE="$TEST_DIR/flores-filled.spec$i.txt"
            GOLD="$RESULT_FOLDER/flores-filled.spec$i.gold"
            cat "$TEST_FILE" | cut -f 1 -d" " > $GOLD

            echo "LID_MODEL $LID_MODEL"
            PREDICTIONS="$RESULT_FOLDER/flores-filled.fasttext.predictions.8.spec$i"
