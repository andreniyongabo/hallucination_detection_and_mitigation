...
EXPERIMENT_NAME="2022-02-18-09-40-msrc-3"
...
FBSEED_DATA_DIR="/private/home/celebio/fbseed20220214_data_3"
...
# NLLB_2022__ISO3=("jpn" "kau_Latn" "mni_Mtei" "dzo" "bod")
# NLLB_2022__FLORES_CODE=("jpn" "kau-Latn" "mni-Mtei" "dzo" "bod")
# NLLB_2022__SEEDDATA_CODE=("" "" "" "dzo" "")
# NLLB_2022__OTHER_JW300_CODE=("" "" "" "" "")
# NLLB_2022__OTHER_LID187_CODE=("" "" "" "" "")
# NLLB_2022__ISO15924=("" "" "" "" "")
...
copy_for_step_0 () {
    for FBSEED_LANG in kau_Latn mni_Mtei
    do
        MONO_CAT_FILE="$TRAIN_DIR/$FBSEED_LANG.cat.txt"
        cp "$FBSEED_DATA_DIR/fbseed20220214.$FBSEED_LANG.txt" $MONO_CAT_FILE
    done

    # JPN:
    xzcat /large_experiments/nllb/mmt/data/monolingual/raw/jpn/xlsum.jpn.xz > "$TRAIN_DIR/jpn.cat.txt"
}

copy_for_step_1 () {
    PREV_CLEANED_TRAIN_FOLDER="/large_experiments/nllb/mmt/lidruns/2022-02-15-23-30-msrc/data/train/"
    for NLLB_LANG in dzo bod
    do
        cp "$PREV_CLEANED_TRAIN_FOLDER/$NLLB_LANG.cleaned.step1.txt" "$TRAIN_DIR/$NLLB_LANG.cleaned.step1.txt"
    done
}
...
prepare_data_combine_train_punct () {
    UNSHUFFLED_TRAIN_FILE="$TRAIN_DIR/all.punct.unshuf.txt"
    > $UNSHUFFLED_TRAIN_FILE
    ALL_TRAIN_FILE="$TRAIN_DIR/all.punct.txt"
    > $ALL_TRAIN_FILE
...
prepare_data_combine_train_punct_long () {
    UNSHUFFLED_TRAIN_FILE="$TRAIN_DIR/all.punct.unshuf.long.txt"
    > $UNSHUFFLED_TRAIN_FILE
    ALL_TRAIN_FILE="$TRAIN_DIR/all.punct.long.txt"
    > $ALL_TRAIN_FILE
    ...
    for i in "${!NLLB_2022__ISO3[@]}"; do
        ...
        TRAIN_FILE="$TRAIN_DIR/$NLLB_LANG.train.mono.punct.txt"
        cat $TRAIN_FILE | awk 'length($0) > 30 { print }' >> $UNSHUFFLED_TRAIN_FILE
    done
    shuf $UNSHUFFLED_TRAIN_FILE > $ALL_TRAIN_FILE
}
...
# flores_filled is lang.flores when flores available, lang.valid when flores not available
prepare_flores_filled_devtest_data () {
    ...
    for i in "${!NLLB_2022__ISO3[@]}"; do
        ...
        if [ -z "$chosen_source" ]
        then
            FLORES_DEV_FILE="$FLORES_DEVTEST_DIR/$FLORES_LANG.devtest"
            if [ -f "$FLORES_DEV_FILE" ]; then
                chosen_source="${chosen_source}-flores"
                cat $FLORES_DEV_FILE | awk -v lang="$LABEL_LANG" '{print "__label__"lang " " $0}' >> $CONCAT_FILE
            fi
        ...
        if [ -z "$chosen_source" ]
        then
            FLORES_DEV_FILE="$FLORES_BETA_DEVTEST_DIR/$FLORES_LANG.devtest"
            if [ -f "$FLORES_DEV_FILE" ]; then
                chosen_source="${chosen_source}-flores_beta"
                cat $FLORES_DEV_FILE | awk -v lang="$LABEL_LANG" '{print "__label__"lang " " $0}' >> $CONCAT_FILE
            fi
...
train_fasttext_8_2 () {
    echo "Training fastText:"
    $FASTTEXT_BIN supervised -input $TRAIN_DIR/all.punct.txt -output $RESULT_FOLDER/model.8.2 \
        -lr 0.8 -minn 2 -maxn 5 -minCount 1000 -epoch 2 -dim 256 -loss softmax -bucket 1000000 -thread 40
}

train_fasttext_8_3 () {
    echo "Training fastText:"
    $FASTTEXT_BIN supervised -input $TRAIN_DIR/all.punct.long.txt -output $RESULT_FOLDER/model.8.3 \
        -lr 0.8 -minn 2 -maxn 5 -minCount 1000 -epoch 2 -dim 256 -loss softmax -bucket 1000000 -thread 40
}
...
train_fasttext_10_1 () {
    mkdir -p "$RESULT_FOLDER/SLURM_LOG"
    sbatch \
        --job-name lid_10_1 \
        --comment lid_10_1 \
        --partition nllb \
        ...
        --ntasks-per-node=1 \
        --time=240 \
        --output $RESULT_FOLDER/SLURM_LOG/train_10_1.log \
        --error $RESULT_FOLDER/SLURM_LOG/train_10_1.err \
        --open-mode append \
        --no-requeue \
        --wrap "srun --unbuffered \
            $FASTTEXT_BIN supervised \
            -input $TRAIN_DIR/all.txt \
            -output $RESULT_FOLDER/model.10.1 \
            -lr 0.8 -minn 2 -maxn 5 -minCount 1000 \
            -epoch 2 -dim 256 \
            -loss softmax \
            -bucket 1000000 \
            -thread 40"
...
train_fasttext_10_2 () {
    mkdir -p "$RESULT_FOLDER/SLURM_LOG"
    MODEL_NUMBER="10_2"
    MODEL_NUMBER_DOT="10.2"
    sbatch \
        --job-name lid_$MODEL_NUMBER \
        --comment lid_$MODEL_NUMBER \
        --partition nllb \
        --nodes=1 \
        --gpus-per-node=0 \
        --cpus-per-task=40 \
        --ntasks-per-node=1 \
        --time=240 \
        --output $RESULT_FOLDER/SLURM_LOG/train_$MODEL_NUMBER.log \
        --error $RESULT_FOLDER/SLURM_LOG/train_$MODEL_NUMBER.err \
        --open-mode append \
        --no-requeue \
        --wrap "srun --unbuffered \
            $FASTTEXT_BIN supervised \
            -input $TRAIN_DIR/all.upsampl.txt \
            -output $RESULT_FOLDER/model.$MODEL_NUMBER_DOT \
            -lr 0.8 -minn 2 -maxn 5 -minCount 1000 \
            -epoch 2 -dim 256 \
            -loss softmax \
            -bucket 1000000 \
...
train_fasttext_10_6 () {
    ...
    sbatch \
        ...
        --wrap "srun --unbuffered \
            ...
            -input $TRAIN_DIR/all.upsampl.txt \
            -output $RESULT_FOLDER/model.$MODEL_NUMBER_DOT \
            -lr 1.2 -minn 2 -maxn 5 -minCount 1000 \
            -epoch 5 -dim 256 \
            -loss softmax \
            -bucket 5000000 \
            -thread 40"
...
train_fasttext_10_7 () {
    ...
    sbatch \
        ...
        --wrap "srun --unbuffered \
            ...
            -input $TRAIN_DIR/all.upsampl.txt \
            -output $RESULT_FOLDER/model.$MODEL_NUMBER_DOT \
            -lr 1.2 -minn 2 -maxn 5 -minCount 100 \
            -epoch 5 -dim 256 \
            -loss softmax \
            -bucket 5000000 \
            -thread 40"
...
train_fasttext_8_2 () {
    ...
    sbatch \
        ...
        --wrap "srun --unbuffered \
            ...
            -input $TRAIN_DIR/all.txt \
            -output $RESULT_FOLDER/model.$MODEL_NUMBER_DOT \
            -lr 0.8 -minn 2 -maxn 5 -minCount 10 \
            -epoch 5 -dim 256 \
            -loss softmax \
            -bucket 1000000 \
            -thread 40"
...
eval_flores_filled_devtest_fasttext_variants () {
    GOLD="$RESULT_FOLDER/flores-filled-devtest.gold"
    ...
    do
        for i in `seq 1 20`
        do
            RESULT_TXT="$RESULT_FOLDER/result.classifiermetrics-flores-filled-devtest.fasttext.$MODEL_MAJOR.$i.txt"
            ...
            then
                echo "LID_MODEL $LID_MODEL"
                PREDICTIONS="$RESULT_FOLDER/flores-filled.devtest.fasttext.predictions.$MODEL_MAJOR.$i"
