...
for lang in LANGS:
    for direction in [f"{lang}-eng", f"eng-{lang}"]:
        ...
        for i, size in enumerate(SIZES):
            for checkpoint in glob.glob(f"{CHECKPOINTS}/{direction}.{size}.*"):
                ...
                try:
                    with open(p_test, "rt") as ftest, open(p_valid, "rt") as fvalid:
                        ...
                        if bleus_valid[i] is None or bleu_valid > bleus_valid[i]:
                            ...
                            link = CHECKPOINTS + f"/{direction}.{size}"
                            try:
                                symlink(checkpoint, link)
