...
# Evaluate all models for a given language and seed size
...
for lang in "${LANGS[@]}"; do
  ...
  for size in "${SIZES[@]}"; do
    ...
    for split in valid test; do
      for checkpoint in $DATA_DIR/checkpoints/$lang-eng.$size.*; do
        ...
        fairseq-generate $data --path $checkpoint/checkpoint_best.pt \
          --task translation_multi_simple_epoch --source-lang $lang --target-lang eng \
          --langs "$lang,eng" --lang-pairs "$lang-eng" --gen-subset $split \
