...
for split in "dev" "test"; do
  for tag in "" ".mined_other"; do
    input_file=/private/home/chau/wmt21/dev_data/${src}_${tgt}_flores/flores.${split}.${src}.tagged${tag}
    for lenpen in 1.0; do
      for beam in 5; do       
        buffer_size=1024
        batch_size=16
        for model_d  in dense.fp16.uf4.entsrc.SPL_temperature.tmp5.shem.ilr1e-07.wu4000.lr0.001.clip0.0.drop0.3.wd0.0.ls0.2.seed1.c10d.det.mt6000.transformer.ELS12.DLS12.encffnx4096.decffnx4096.E1024.H16.NBF.ATTDRP0.2.RELDRP0.2.ngpu32        dense.fp16.uf4.entsrc.SPL_temperature.tmp5.shem.ilr1e-07.wu4000.lr0.001.clip0.0.drop0.3.wd0.0.ls0.2.seed1.c10d.det.mt6000.transformer.ELS12.DLS12.encffnx4096.decffnx4096.E1024.H16.NBF.ATTDRP0.3.RELDRP0.3.ngpu32; do
        out_dir=/checkpoint/chau/wmt21/bitext_bt/ha_en.bitext_bt.32k/$model_d
        model=$out_dir/checkpoint_best.pt
        langs="en,${src}"
        lang_pairs=${src}-en
        spm=${datadir}/sentencepiece.32000.model
        prefix=flores_${split}${tag}.single.${lenpen}.${beam}
        ...
        sacrebleu -l ${src}-${tgt} /private/home/chau/wmt21/dev_data/${src}_${tgt}_flores/flores.${split}.${tgt} < ${out_dir}/${prefix}.output > ${out_dir}/${prefix}.results
      done
      done
    done
  done
